...
;(function($) {
  ...
  var cartData,
    ...
    orderform = '<p class="jqcart-cart-title">Контактная информация:</p><form class="jqcart-orderform"><p><label>Battle tag</label><input type="text" name="user_name"></p><p><label>Armory Link</label><input type="text" name="user_phone"><br><span style="color:#a6acaf">Перейдите по адресу <a style="color: #fff" href="http://eu.battle.net/wow/en/" target="_blank">http://eu.battle.net/wow/en/</a>. Вы можете найти окно поиска в правом верхнем углу страницы. Введите имя своего персонажа, выберите его и скопируйте ссылку (в поле поиска)</span></p><p><label>Email:</label><input type="text" name="user_mail"></p><p><label><p><label>Коментарий:</label><textarea name="user_comment"></textarea></p><p><input type="submit" value=""><input type="reset" value=""></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    init: function(o) {
        ...
        for (var key in cartData) {
          if (cartData.hasOwnProperty(key)) {
            totalCnt += cartData[key].count;
						$(opts.buttons).filter('[data-id="' + cartData[key].id + '"]').prop('checked', true);
          }
        }
        visibleLabel = true;
      }
      ...
      $(document)
        .on('change', opts.buttons, actions.addToCart)
    ...
    addToCart: function(e) {
      ...
      if (cartData.hasOwnProperty(itemData.id)) {
				actions.changeTotalCnt(-cartData[itemData.id].count);
				delete cartData[itemData.id];
      } else {
        itemData.count = 1;
    ...
    delFromCart: function() {
      ...
      actions.changeTotalCnt(-cartData[itemId].count);
			$(opts.buttons).filter('[data-id="' + itemId + '"]').prop('checked', false);
    ...
    hideCart: function() {
      $('.jqcart-layout').fadeOut('fast', function() {ыы
        $(this).remove();
      });
      return false;
    },
    sendOrder: function(e) {
      ...
      if ($.trim($('[name=user_name]', $that).val()) === '' || $.trim($('[name=user_phone]', $that).val()) === '') {
        $('<p class="jqcart-error">Пожалуйста, укажите свой Battle Tag и Armory link!</p>').insertBefore($that).delay(3000).fadeOut();
        return false;
      }
      $.ajax({
        url: opts.handler,
